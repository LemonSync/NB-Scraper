 ...
 * @author wolep
 */
export async function getYoutubePost(url: string): Promise<NBScraperResponse<YouTubePostData>> {
  try {
    ...
    if (!bpr) {
      return createErrorResponse('Failed to parse the main post renderer object.', {
        type: ScraperErrorType.PARSE_ERROR
      });
    }
    ...
    if (votePost) {
        ...
        images = votePost.map((v: any) => {
            const text = v.text.runs[0].text;
            let imageUrl = v.image?.thumbnails || null;
            if (imageUrl) {
                imageUrl = imageUrl.map((thumb: any) => thumb.url).pop();
                isVoteImage = true;
            }
        ...
        images = multipleImagePost.map((v: any) => ({
            url: v.backstageImageRenderer.image.thumbnails.map((thumb: any) => thumb.url).pop(),
            text: null
        }));
    } else if (singleImagePost) {
        postType = "singleImage";
        images = [{
            url: singleImagePost.map((v: any) => v.url).pop(),
            text: null
        }];
    ...
    const result: YouTubePostData = {
        ...
        authorUrl: new URL(url).origin + bpr.authorEndpoint.commandMetadata.webCommandMetadata.url,
        publishTime: bpr.publishedTimeText.runs[0].text,
        text: bpr.contentText.runs.map((run: any) => run.text).join(''),
